...
create_node_files() {
  ...
  echo "Creating NodeJS Express with Typescript..."
  ...
  clear
#  cd ../projects/ || read -p "Error"
#  mkdir "${project_name}" && cd "${project_name}" || exit 1
  echo "PORT=5001" >> .env
  echo "init..."
  npm init -y
  echo "npm i ..."
  npm i express dotenv && npm i -D nodemon ts-node concurrently && npm i -D typescript @types/express @types/node && npm i --save-dev tsc-alias tsconfig-paths
...
#  update_package_json_function 'scripts": {' '  "build": "npx tsc",' "package.json" .
  ...
  echo "" >> index.ts
  echo "interface ResObj {" >> index.ts
  echo "  status_code: number;" >> index.ts
  echo "  status: string;" >> index.ts
  echo "}" >> index.ts
  echo "" >> index.ts
  echo 'app.get("/", (req: Request, res: Response) => {' >> index.ts
  echo '  const res_obj: ResObj = {status_code: 200, status: "Welcome to Express + TypeScript Server!"}' >> index.ts
  echo '  res.status(res_obj.status_code).json(res_obj)' >> index.ts
  echo '})' >> index.ts
  echo "" >> index.ts
  echo 'app.get("/health", (req: Request, res: Response) => {' >> index.ts
  echo '  TestHelperFunction()' >> index.ts
  echo '  TestServiceFunction()' >> index.ts
  echo '  const res_obj: ResObj = {status_code: 200, status: "Running"}' >> index.ts
  echo '  res.status(res_obj.status_code).json(res_obj)' >> index.ts
  echo '})' >> index.ts
  echo "" >> index.ts
...
#  cd ../../test || echo "Error on cd cmd"

  cat ../../test/Dockerfile >> Dockerfile
  cat ../../test/docker-compose.yaml >> docker-compose.yaml
